<!--
  CameraDetectionPage.vue — 摄像头实时检测版本

  说明：本页面为摄像头实时检测独立页面，与 DetectionPage.vue（上传检测版本）并行存在。
  功能：通过浏览器 getUserMedia() 获取摄像头画面，WebSocket 实时发送帧到后端进行推理，
  支持 GPU/CPU 双模式切换，实时显示 FPS、目标数、推理耗时、类别分布等统计信息。
-->
<template>
