...
async function handleSend({ text, files }) {
  ...
  const apiEndpoint = agentStore.agentMode === 'multi' ? '/api/chat/multi-agent' : '/api/chat/messages/stream'
  const stop = streamChat(apiEndpoint, requestBody, {
    onMessage: (data) => {
      ...
      switch (data.type) {
        ...
        case 'annotation_image':
          ...
          lastMsg.detectionResult = {
            ...
            fire_object_count: data.fire_object_count,
            smoke_object_count: data.smoke_object_count,
...
onMounted(async () => {
  ...
  if (token) {
    ...
    await agentStore.fetchAgentMode()
  }
...
$color-main-dark: #4A90D9;
$color-main-light: #87CEEB;
