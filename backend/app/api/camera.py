...
# 心跳间隔（秒），用于 WebSocket 保活，防止代理/防火墙断开空闲连接
...
async def camera_websocket(websocket: WebSocket):
    ...
    # 心跳保活：记录最后一次 pong 时间，用于心跳任务判断客户端是否存活
    last_pong_at = time.monotonic()
    heartbeat_task: asyncio.Task | None = None

    async def send_heartbeats() -> None:
        """定期发送 ping 保持 WebSocket 连接活跃，防止代理/防火墙断开空闲连接。
        
        注意：本函数仅负责保活，不做空闲超时检测。
        空闲超时由外层的 asyncio.wait_for 负责。
        """
        while True:
