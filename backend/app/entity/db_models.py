...
数据库模型定义（共 17 张表）
表结构总览：
  用户权限：users, user_configs, roles, permissions, user_roles, role_permissions
...
class User(Base):
    ...
    nickname = Column(String(50), nullable=True, comment="昵称（用户自定义显示名）")
...
class UserConfig(Base):
    """用户配置表 — 存储用户个性化设置（如 Agent 模式）"""
    __tablename__ = "user_configs"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("users.id"), unique=True, nullable=False, comment="用户ID")
    agent_mode = Column(String(20), default="single", nullable=False, comment="Agent模式: single/multi")
    created_at = Column(DateTime, default=datetime.now, comment="创建时间")
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now, comment="更新时间")
...
class RefreshToken(Base):
    """Refresh Token 表 — 用于长期保持登录状态，支持撤销"""
    __tablename__ = "refresh_tokens"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True, comment="所属用户")
    token = Column(String(255), unique=True, nullable=False, index=True, comment="Token 字符串")
